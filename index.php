...
if(isset($_POST['signbtn'])){
    ...
    $gender = $_POST['gender'];
    #inser query PostgreSQL
    $res = pg_query($db,"INSERT INTO  user_register(firstname,surname,mob_emai,dob,gender) VALUES('$firstname','$surname','$mobemail','$dob','$gender')");
    #get last insert id
    $last_id = pg_query($db,"SELECT max(register_id ) FROM user_register");
    $data =  pg_fetch_array($last_id);
    $lastid = $data[0];
    #insert username and pass with foreign key
    pg_query($db,"INSERT INTO  user_login(user_id,user_name,password) VALUES('$lastid',' $mobemail','$password')");
    if ($res) {
...
if(isset($_POST['login'])){
    ...
    $log_qry = pg_query($db,"SELECT * FROM user_login WHERE password = '$pass'");
